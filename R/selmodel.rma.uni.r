selmodel.rma.uni <- function(x, type, alternative="greater", prec, delta, steps, verbose=FALSE, digits, control, ...) {
   ...
   if (k < p + ifelse(x$method == "FE" || x$tau2.fix, 0, 1) + sum(is.na(delta)))
   ...
   if (!x$tau2.fix && x$method != "FE" && !isTRUE(ddd$skiphet)) {
   ...
   parms <- p + ifelse(x$method == "FE" || x$tau2.fix, 0, 1) + sum(is.na(delta.val))
