cooks.distance.rma.mv <- function(model, ...) {
   ...
   for (i in seq_len(x$k.f)[x$not.na]) {
      ...
      res <- try(suppressWarnings(rma.mv(x$yi.f[-i], x$V.f[-i,-i], W=x$W.f[-i,-i], mods=cbind(x$X.f[-i,]), random=x$random, struct=x$struct, intercept=FALSE, method=x$method, tdist=x$knha, R=x$R, Rscale=x$Rscale, data=lapply(x$mf.r, function(x) x[-i,,drop=FALSE]), sigma2=ifelse(x$vc.fix$sigma2, x$sigma2, NA), tau2=ifelse(x$vc.fix$tau2, x$tau2, NA), rho=ifelse(x$vc.fix$rho, x$rho, NA), gamma2=ifelse(x$vc.fix$gamma2, x$gamma2, NA), phi=ifelse(x$vc.fix$phi, x$phi, NA), control=x$control)), silent=TRUE)
